    ...
    sig
      ...
      module Tagged :
        sig
          ...
          val declpt : tag:'a -> p:Pt.t -> n:N.t -> 'a s
        end
      ...
      val declpt : p:Pt.t -> n:N.t -> untagged
      val mid : Pt.t -> Pt.t -> Pt.t
      val bezier_midpoint : Pt.t -> Pt.t -> Pt.t -> Pt.t -> Pt.t
      val anchor_of : 'a s -> Pt.t
      val bouquet_of_segments : Pt.t -> Pt.t -> N.t list -> untagged list
      ...
      val arrow :
        start:N.t -> finish:N.t -> sty:Arrow.style -> layout -> layout
      val smart_arrow :
        start:N.t -> finish:N.t -> sty:Arrow.style -> layout -> layout
                                                                  
      (* val align_right_centered_vector : Bbox.t -> Bbox.t -> float -> Pt.t *)
      (* val align_bottom_centered_vector : Bbox.t -> Bbox.t -> float -> Pt.t *)
