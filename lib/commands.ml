...
let ubezier_control_points ~p1 ~p2 ~angle =
  ...
  let out_angle = n_angle +. angle in
  let in_angle =
    n_angle -. angle
    (* +. Tools.pi *. 0.5 -. angle *)
  in
...
let text ~pos ~size ~text = mktag (Text { pos; text = Ctext.create ~size text })
  ...
  Format.(
    ...
    | Image { pos; image } ->
        fprintf
          ...
          (Image.xsize image)
          (Image.ysize image)
    | DeclPt { pt; name } -> fprintf fmtr "DeclPt(%a, %a)" Pt.pp pt Name.pp name
