...
let segment_intersection (p1, p2) (p3, p4) =
  ...
  if near_zero (dot -. 1.0) then
    (* colinear vectors *)
    None
  else if near_zero u1x && near_zero u2x then
    None
  else if near_zero u1x then
    let k2 = (p1x -. p3x) /. u2x in
    let k1 = p3y -. p1y +. k2 *. u2y in
    if k1 <= epsilon || k1 >= (dist p1 p2) || k2 <= epsilon || k2 >= (dist p3 p4) then
      None
    else
    ...
    let k2 = (p3x *. (u1y /. u1x) +. p1y -. p3y) /. (u2y -. u2x *. u1y /. u1x) in
    if k2 <= epsilon || k2 >= (dist p3 p4) then
      None
    else
      let k1 = (k2 *. u2x +. p3x -. p1x) /. u1x in
      if k1 <= epsilon || k1 >= (dist p1 p2) then
        None
      else
        Some(plus p3 (pt (k2 *. u2x) (k2 *. u2y)))

let point_in_box p bbox =
  let { x = px; y = py } = p in
  Bbox.(
    bbox.mins.x <= px && px <= bbox.maxs.x &&
      bbox.mins.y <= py && py <= bbox.maxs.y
  )
...
let orient_of_segment ({ x = p1x; y = p1y }, { x = p2x; y = p2y }) =
  let dx = p2x -. p1x in
  let dy = p2y -. p1y in
  let xtest = dx > 0.0 in
  let ytest = dy > 0.0 in
  if xtest && ytest then
    UpRight
  else if xtest && not ytest then
    DownRight
  else if not xtest && not ytest then
    DownLeft
  else
    UpLeft
...
let intersect_box box seg =
  ...
  | (Some s, Some w, None, None) ->
     Some(SW, s, w)
  | (Some s, None, Some n, None) ->
     Some(NS, n, s)
  | (Some s, None, None, Some e) ->
     Some(SE, s, e)
  | (None, Some w, Some n, None) ->
     Some(NW, n, w)
  | (None, Some w, None, Some e) ->
     Some(WE, w, e)
  | (None, None, Some n, Some e) ->
     Some(NE, n, e)
  | _ ->
     let _ = Printf.printf "SmartPath.intersect_box: warning, borderline case ignored\n%!" in
...
let solution shift box pb orient =
  ...
  | (WE, w, e) ->
     ...
     (match orient with
      | UpRight
      | DownRight ->
         ( c_sol, barycenter c_sol d_sol, d_sol )
      | _ ->
         ( b_sol, barycenter b_sol a_sol, a_sol ))
